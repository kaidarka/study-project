...
Для JSX-ветвления можно использовать компонент `ToggleFeatures`:
...
- для вычисляемых значений и колбэков использовать `toggleFeatures`, для JSX-рендера - `ToggleFeatures`
...
- удаляет ветвление у `toggleFeatures(...)`
- удаляет ветвление у `<ToggleFeatures ... />`
- оставляет только выбранный вариант (`on` или `off`) по имени флага

Ограничения:

- `name` должен быть строковым литералом (например, `name="isNewFeatureEnabled"` или `name: 'isNewFeatureEnabled'`)
- `on`/`off` должны быть переданы явно в том же вызове/теге
- если флаг вычисляется динамически, такие места нужно править вручную
